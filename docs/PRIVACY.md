...
Transcriber records meeting audio on your Mac and turns it into a Markdown transcript. This document is the complete description of what data the app handles, where it lives, and what leaves your device.

## What stays on your Mac

- **Raw audio captures** — `mic.m4a` and `system.m4a`, one per session, written to your output folder. Default output folder: `~/Documents/Transcriber/`.
...
- **Settings** — engine choice, output folder, keep-raw-streams, AEC enable, privacy acknowledgement. Stored in macOS UserDefaults under `transcriber.settings.v1`.
...
- **Logs** — `~/Library/Logs/TranscriberApp/` contains lifecycle and engine logs. Per Apple's `os_log` privacy contract, file paths are marked `.private` and never appear in shared logs.
...
- **Audio** — the full mixed audio of each session, encoded as AAC m4a, posted as a multipart form to ElevenLabs Scribe.
...
The "Export Diagnostics…" menu item writes a JSON file to `~/Library/Logs/TranscriberApp/diagnostics-<timestamp>.json`. The file's complete schema is:
...
To remove all Transcriber data:

1. Quit the app.
2. Delete `~/Documents/Transcriber/` (or whichever folder you configured as output).
...
4. Delete `~/Library/Preferences/com.szymonsypniewicz.transcriber.plist`.
5. Delete `~/Library/Logs/TranscriberApp/`.
6. Drag the app to the Trash.

## Privacy acknowledgement

On first launch, Transcriber presents a modal explaining what data leaves the device. Recording is gated until you click "I understand." The modal cannot be dismissed via the title-bar close button — your only options are to acknowledge or quit the app. The acknowledgement is stored in `transcriber.settings.v1` (UserDefaults) as a one-way flag; the Settings UI cannot demote it back to false (enforced by `SettingsStore.commit`).
