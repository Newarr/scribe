...
These items are unresolved drift between SPEC.md and the current implementation. They are NOT spec changes. The spec captures the intended behavior and the code needs to align.

- **Start prompts can suppress an app, but not a recurring meeting.** Code now keeps the main prompt choices to `Start Recording` and `Not now`. App-level `Stop detecting [App] for 30 minutes` exists in menu-bar recovery. The modal still lacks its own closed `More options ▾` disclosure, and `Stop asking about this meeting` is not implemented. **Code action:** add the disclosure to the modal, persist recurrence-series suppressions, and route that state through Settings.
- **Quiet Meetings settings panel is missing.** Spec: `Settings → Quiet Meetings` lists suppressed recurring series with a `Re-enable` action per series. Code: no UI, no persisted recurring-series suppression. **Code action:** add the Settings panel after recurring-series suppression exists.
- **Transcript contract still differs from the spec.** Code can write `schema: transcriber/v1` in a pending stub, writes `started_at` and `ended_at` aliases, does not plumb sanitized calendar notes, and does not fully populate organizer or location from EventKit. Failure transcripts also use a different body shape from success, and speaker blocks are emitted per utterance instead of grouped consecutive same-speaker blocks. **Code action:** align `CaptureSession.writeTranscriptStub`, `TranscriptWriter`, `MetadataJSONWriter`, `TranscriptContext`, and EventKit mapping with the Markdown Contract.
- **Recording trust UI is incomplete.** Code tracks live `micLevel` and `systemLevel`, but the active popover still shows a decorative waveform instead of data-driven `MIC` and `SYS` meters. The status item also lacks elapsed time plus live channel indicators. **Code action:** render the tracked levels in both places and keep the silent-channel warning behavior visible.
- **Recents are not yet the 5-item shortcut described here.** The main popover currently refreshes at 3 items, and rows expose `Open transcript` as the primary click plus `Open folder` in the context menu. Spec requires 5 items with inline `Open Folder` and `Open Transcript` actions. **Code action:** raise the popover limit to 5 and add the inline actions without turning Recents into a transcript browser.
- **Storage warning and cleanup surfaces are partial.** Code detects synced storage warnings, but the folder-selection and recording-start paths do not yet show the one-time third-party provider acknowledgement. Settings also lacks `Delete all audio (keep transcripts)`. **Code action:** add the acknowledgement path and the Storage cleanup action while keeping iCloud as a passive note.

No longer drift: default output root is `~/Scribe/` in `AppDelegate.defaultSettingsFallback`.

Recommended landing order: **start-prompt disclosure and recurring suppression → Quiet Meetings panel → transcript contract → recording trust UI → Recents → storage warnings and cleanup**.
