...
V1.0-rc1 is **code-complete**, not V1-shippable. The user (you) validates every box below before tagging `v1.0.0`. Until then, the build is `v1.0.0-rc1`.
...
For each section:
1. Run the steps verbatim against a clean install (`brew uninstall --cask transcriber && brew install --cask transcriber`) or, in development, against an ad-hoc-signed build (`xcodebuild -project TranscriberApp/TranscriberApp.xcodeproj`).
...
- [ ] `xcodebuild -project TranscriberApp/TranscriberApp.xcodeproj -scheme TranscriberApp -configuration Release build` produces `TranscriberApp.app`.
...
- [ ] `~/Library/Logs/TranscriberApp/diagnostics-*.json` is created.
...
- [ ] `scripts/release.sh 1.0.0-rc1` succeeds end-to-end on a Mac with the Developer ID cert and `transcriber-notary` keychain profile.
- [ ] Result `TranscriberApp.app` passes `spctl --assess`:
  ```
  /Volumes/.../TranscriberApp.app: accepted
