...
Transcriber is a single-user macOS desktop app. The threat model focuses on:
...
- Adversarial process injection. Transcriber relies on macOS hardened runtime + sandbox to isolate from other apps.
...
V1.0-rc1 ships with hardened runtime enabled in xcodegen (`ENABLE_HARDENED_RUNTIME: YES`). The entitlements file `TranscriberApp/TranscriberApp/TranscriberApp.entitlements` contains:
