...
V1.0-rc1 is the code-complete state. Tagging `v1.0.0` requires the user to validate against `docs/TESTING.md` (Phase υ).
...
- App-specific password for `notarytool`, stored in Keychain via `xcrun notarytool store-credentials transcriber-notary --apple-id <email> --team-id <team>`.
- `xcodegen` for project regeneration: `brew install xcodegen`.
- `wrangler` is NOT used for this app (Transcriber is local-only; no cloud deploys).
...
3. `xcodebuild archive` produces `TranscriberApp.xcarchive`.
...
5. `xcrun notarytool submit ... --keychain-profile transcriber-notary --wait` notarizes; on success the staple is attached.
...
- **Direct download** — `releases.transcriber.app/v1.0.0/Transcriber-v1.0.0.dmg`. Hash + signing fingerprint published in the release notes.
- **Homebrew Cask** — Phase τ ships `Casks/transcriber.rb.template`. The release script substitutes version + sha256 and submits to the user's tap.
...
2. Run `spctl --assess --verbose=4 /Volumes/Transcriber/Transcriber.app` — should report `accepted` with `source=Notarized Developer ID`.
3. Drag-install + launch on a clean Mac. Click through the privacy acknowledgement, hit Record, verify the menu bar transitions to "Stop." Run the V1 acceptance walkthrough in `docs/TESTING.md`.
...
Releases are immutable; to revert, publish a new patch release with the offending change reverted. The download URL stays at the latest version; users on older versions will pick up the fix via Homebrew (`brew upgrade --cask transcriber`) or by re-downloading the DMG.
