...
class PrivacyAcknowledgementController {
  ...
  func present() {
    ...
    host.contentView = NSHostingView(
      rootView: PrivacyAcknowledgementView(
    ...
    VStack(alignment: .leading, spacing: 0) {
      ...
      VStack(alignment: .leading, spacing: 18) {
        privacyRow(
          ...
          detail:
            "Sits in your menu bar. Captures mic and system audio and saves it to your Scribe folder. Audio files always stay on your Mac."
        )
        privacyRow(
          state: .ready,
          label: "TRANSCRIPTION",
          ...
          detail:
            "Local (Cohere): transcription runs entirely on your Mac — no audio leaves the device. Cloud (ElevenLabs): mixed audio is uploaded to ElevenLabs for transcription; if you have Calendar access, title and attendee keyterms may also be sent."
        )
        privacyRow(
          state: .ready,
          label: "MARKDOWN",
          ...
          detail:
            "Each session is plain Markdown with frontmatter saved locally. Drop the folder into Obsidian, Cursor, Claude, or any tool that reads a directory."
        )
      }
      ...
      HStack {
        Button("Read full privacy details") {
          if let url = URL(
            string: "https://github.com/Newarr/scribe/blob/main/docs/user/PRIVACY.md")
          {
