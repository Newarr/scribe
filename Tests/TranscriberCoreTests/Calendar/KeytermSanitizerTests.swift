...
final class KeytermSanitizerTests: XCTestCase {
  func testStripsURLsAndDomains() {
    ...
    XCTAssertFalse(
      s.contains(where: { $0.contains("google.com") }), "domains must be stripped: \(s)")
    XCTAssertTrue(s.contains("Faris"))
  }

  func testStripsBareMeetingUrlsWithUnknownTLDs() {
    let raw = [
      "Vendor",
      "sync",
      "meet.vendor.cloud/room-abc",
      "Meet",
      "at",
      "jitsi.si/team-room",
      "example.internal/secret-room",
      "Ramp.network",
    ]
    let s = KeytermSanitizer.sanitize(raw)
    XCTAssertTrue(s.contains("Vendor"), "regular title words must survive: \(s)")
    XCTAssertTrue(s.contains("sync"), "regular title words must survive: \(s)")
    XCTAssertTrue(
      s.contains("Ramp.network"), "plain company/domain terms without path shape may survive: \(s)")
    XCTAssertFalse(
      s.contains(where: { $0.contains("/") }), "bare meeting URL paths must be stripped: \(s)")
    XCTAssertFalse(
      s.contains(where: { $0.lowercased().contains("vendor.cloud") }),
      "unknown-TLD meeting host must be stripped: \(s)")
    XCTAssertFalse(
      s.contains(where: { $0.lowercased().contains("jitsi.si") }),
      "short-TLD meeting host must be stripped: \(s)")
    XCTAssertFalse(
      s.contains(where: { $0.lowercased().contains("internal") }),
      "private-TLD host/path must be stripped: \(s)")
    XCTAssertFalse(
      s.contains(where: { $0.lowercased().contains("room") }),
      "meeting room IDs must be stripped with the link token: \(s)")
  }
  ...
  func testStripsPhoneNumbers() {
    ...
    let s = KeytermSanitizer.sanitize(raw)
    XCTAssertFalse(
      s.contains(where: { $0.contains("555") }), "phone numbers must be stripped: \(s)")
  }
  ...
  func testStripsLabelEqualsValue() {
    ...
    let s = KeytermSanitizer.sanitize(raw)
    XCTAssertFalse(
      s.contains(where: { $0.lowercased().contains("987654") }),
      "PIN=value tokens must be stripped: \(s)")
    XCTAssertTrue(s.contains("Faris"))
  }
  ...
  func testKeepsRegularNamesAndWords() {
    ...
    let s = KeytermSanitizer.sanitize(raw)
    XCTAssertEqual(
      Set(s), Set(["Project", "Rocket", "Faris", "Riaz", "Q4", "review"]),
  ...
  func testCalendarEventKeytermsExcludeSecrets() {
    ...
    XCTAssertFalse(
      terms.contains(where: { $0.contains("@") }), "attendee email must be stripped: \(terms)")
    XCTAssertFalse(
      terms.contains(where: { $0.contains("google.com") }), "URLs must be stripped: \(terms)")
