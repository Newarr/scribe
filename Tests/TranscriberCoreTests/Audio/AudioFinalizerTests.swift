...
final class AudioFinalizerTests: XCTestCase {
  ...
  func testStreamingFinalizeHandlesLongDurationWithoutMemoryBlowup() async throws {
    ...
    XCTAssertLessThan(
      elapsed, 30.0, "30s of audio must finalize in well under 30s; got \(elapsed)s")
  ...
  func testStreamingHandlesUnequalLengthInputs() async throws {
    ...
    XCTAssertGreaterThan(
      rms, 0.1,
      "tail (sys-only, single-active) should retain tone; RMS=\(rms) — finalizer may be attenuating single-active or writing wrong side"
    )
  }
  ...
  func testStreamingMemoryStaysBoundedAt90s() async throws {
    ...
    XCTAssertLessThan(
      delta, 80 * 1024 * 1024,
      "streaming finalize on 90s fixture peaked at \(mb) MB above baseline — likely regressed to whole-file reads"
    )
  }
  ...
  func testStreamingFailureLeavesExistingOutputIntact() async throws {
    ...
    do {
      try await AudioFinalizer.finalize(
        mic: badMic, system: sysURL, output: outURL, sampleRate: 48000)
    ...
    XCTAssertTrue(
      ...
      "previous good output must survive a failed retry")
  ...
  func testPTSTimelinePreservesInterBufferGap() async throws {
    ...
    XCTAssertLessThan(
      ...
      "200ms logged PTS gap should remain silent instead of compressed")
    XCTAssertGreaterThan(
      rms(samples, start: 15_000, count: 2_000), 0.05,
      "second chunk should begin after the logged gap")
  }
  ...
  func testPTSTimelineHonorsSubChunkOffset() async throws {
    ...
    XCTAssertLessThan(
      firstEnergy, 3_200,
      "50ms offset should land near frame 2400, not rounded to one 4800-frame chunk; got \(firstEnergy)"
    )
  }
  ...
  func testPTSTimelineNormalizesNonZeroFirstPTS() async throws {
    ...
    let ptsURL = tmp.appendingPathComponent("pts-absolute.jsonl")
    let outURL = tmp.appendingPathComponent("audio-absolute.m4a")
    try writeAACPCM(to: micURL, sampleRate: 48000, duration: 0.2) { ptr, frames, _ in
      for i in 0..<frames { ptr[i] = Float(0.45 * sin(2 * .pi * 440 * Double(i) / 48000.0)) }
    }
    try writeAACSilence(to: sysURL, durationSec: 0.2)
    ...
    let samples = try readSamples(from: outURL)
    XCTAssertLessThan(
      samples.count, 40_000,
      "absolute 100s host-clock PTS must be normalized instead of producing huge leading silence")
    XCTAssertGreaterThan(rms(samples, start: 1_000, count: 2_000), 0.05)
    XCTAssertLessThan(rms(samples, start: 6_000, count: 6_000), 0.02)
    XCTAssertGreaterThan(rms(samples, start: 15_000, count: 2_000), 0.05)
  }

  func testPTSTimelineUsesBoundedStreamingForLongPTSLog() async throws {
    let micURL = tmp.appendingPathComponent("mic-long-pts.m4a")
    let sysURL = tmp.appendingPathComponent("system-long-pts.m4a")
    let ptsURL = tmp.appendingPathComponent("pts-long.jsonl")
    let outURL = tmp.appendingPathComponent("audio-long-pts.m4a")
    let duration = 90.0
    try writeAACSilence(to: micURL, durationSec: duration)
    try writeAACSilence(to: sysURL, durationSec: duration)
    let chunkFrames = 4800
    let chunks = Int(duration * 48000.0 / Double(chunkFrames))
    var entries: [PTSLogEntry] = []
    for i in 0..<chunks {
      let pts = 10_000.0 + Double(i) * 0.1
      entries.append(
        PTSLogEntry(stream: "mic", ptsSeconds: pts, sampleCount: chunkFrames, sampleRate: 48000))
      entries.append(
        PTSLogEntry(stream: "system", ptsSeconds: pts, sampleCount: chunkFrames, sampleRate: 48000))
    }
    try writePTSLog(to: ptsURL, entries: entries)
    ...
    let delta = await sampler.peak - baseline
    XCTAssertLessThan(
      delta, 80 * 1024 * 1024,
      "PTS timeline finalizer should stream instead of allocating whole-session buffers")
    let asset = AVURLAsset(url: outURL)
    let seconds = CMTimeGetSeconds(try await asset.load(.duration))
    XCTAssertGreaterThan(seconds, 80)
  ...
  func testMalformedMiddlePTSLogThrowsAndPreservesExistingOutput() async throws {
    ...
    XCTAssertTrue(
      leftovers.isEmpty, "failed PTS parse should not leave inflight temp files: \(leftovers)")
  }
...
private func readResidentBytes() -> Int {
